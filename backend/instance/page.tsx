...
export default function Dashboard() {
  ...
  const handleBet = () => {
    if (selectedTeam === null || selectedTeam === undefined || !betAmount || isNaN(parseFloat(betAmount)) || parseFloat(betAmount) <= 0) {
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center">
        <div className="text-center text-white">
          ...
          {status.winning_team_id !== null && (
            <p className="text-yellow-400">获胜队伍：队伍 {status.winning_team_id}</p>
          )}
        </div>
      </div>
    );
                    ...
                    <Dialog>
                      ...
                      <DialogContent className="bg-slate-800 border-slate-700">
                        ...
                        <div className="space-y-4">
                          ...
                          <Button className="w-full bg-yellow-500 hover:bg-yellow-600 text-black" onClick={handleBet}>
                            确认下注
                          </Button>
                        </div>
                      </DialogContent>
